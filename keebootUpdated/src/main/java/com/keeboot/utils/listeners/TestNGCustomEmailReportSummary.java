...
public class TestNGCustomEmailReportSummary implements IReporter {

    private static final Logger L = Logger.getLogger(TestNGCustomEmailReportSummary.class);
    ...
    protected PrintWriter createWriter(String outdir) throws IOException {
         ...
         * }
         */
        return new PrintWriter(new BufferedWriter(new FileWriter(new File(outdir, "CustomEmailReportSummary.html"))));
    ...
    public void generateSuiteSummaryReport(List<ISuite> suites) {
        ...
        startSummaryRow("MGM Automated Test Execution Report");
        out.print("<tr>");
        ...
        int qty_skip = 0;
        int qty_fail = 0;
        int qty_grand_total = 0;
        long time_start = Long.MAX_VALUE;
        long time_end = Long.MIN_VALUE;
        testIndex = 1;
        for (ISuite suite : suites) {
            // To be printed only if more than one Suite is there in TestNG XML
            // if (suites.size() >= 1) {
            if (suites.size() > 1) {
            ...
            for (ISuiteResult r : tests.values()) {
                ...
                q_pass = getMethodSet(overview.getPassedTests(), suite).size();
                qty_pass_m += q_pass;
                /*
                 ...
                 */
                q_skip = getMethodSet(overview.getSkippedTests(), suite).size();
                qty_skip += q_skip;
                q_fail = getMethodSet(overview.getFailedTests(), suite).size();
                qty_fail += q_fail;

                q_total = q_pass + q_skip + q_fail;

                summaryCell(q_total, Integer.MAX_VALUE);
                summaryCell(q_pass, Integer.MAX_VALUE);
                summaryCell(q_skip, Integer.MAX_VALUE);
                summaryCell(q_fail, Integer.MAX_VALUE);
        ...
        if (qty_tests > 1) {
            out.println("<tr style=\"background-color:Gray; color: white; class=\"total\"><td>Total</td>");
            qty_grand_total = qty_pass_m + qty_skip + qty_fail;
            summaryCell(qty_grand_total, Integer.MAX_VALUE);
            summaryCell(qty_pass_m, Integer.MAX_VALUE);
            // summaryCell(qty_pass_s, Integer.MAX_VALUE);
            summaryCell(qty_skip, Integer.MAX_VALUE);
            summaryCell(qty_fail, Integer.MAX_VALUE);
            // summaryCell(" ", true);
